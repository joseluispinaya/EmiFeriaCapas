...
$(document).ready(function () {
    const usuarioEstu = sessionStorage.getItem('usuEstu');
    ...
    try {
        const usua = JSON.parse(usuarioEstu);
        // mostrar la imagen y nombre del usuairo 
        $("#imgMaEsts").attr("src", usua.ImagenEstUrl || "/Imagenes/sinimagen.png");
        $("#txtApellidosEst").text(usua.Apellidos);
    } catch (error) {
...
function EjecutarCierreSesion() {
    $.ajax({
        ...
        url: "InicioEst.aspx/CerrarSesion",
        type: "POST",
        data: "{}",
        dataType: "json",
