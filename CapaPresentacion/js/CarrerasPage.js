...
function cargarGradosAcade() {
    ...
    $.ajax({
        ...
        error: function (xhr, ajaxOptions, thrownError) {
            ...
            mostrarAlerta("Error", "No se pudieron cargar los Grados Académicos.", "error", "btn btn-danger");
        }
    });
}
...
$('#tbCarreras tbody').on('click', '.btn-detalle', function () {
    ...
    let data = tablaData.row(fila).data();
    const textoSms = `Detalles de la carrera ${data.Nombre}.`;
...
$("#btnGuardarReg").on("click", function () {
    ...
    if ($("#cboGrados").val() === "") {
        ToastMaster.fire({ // O ToastMaster.fire según la opción que dejaste
            icon: 'warning',
            title: 'Debe seleccionar un Grado Académico'
        });
        $("#cboGrados").focus();
        habilitarBoton();
        return;
    ...
    const url = idEditar === 0 ? "CarrerasPage.aspx/Guardar" : "CarrerasPage.aspx/Editar";
